...
public class Shop
{
    ...
    public Shop()
    {
        ...
        {
            new Itemlist(" 1번 아이템", "방어력 +2", 0, 2, 1000) { Quantity = 0 },
    new Itemlist(" 2번 아이템", "방어력 +6", 0, 6, 1500) { Quantity = 0 },
    new Itemlist(" 3번 아이템", "방어력 +10", 0, 10, 3000) { Quantity = 0 },
    new Itemlist(" 4번 아이템", "공격력 +2", 2, 0, 500) { Quantity = 0 },
    new Itemlist(" 5번 아이템", "공격력 +5", 5, 0, 1000) { Quantity = 0 },
    new Itemlist(" 6번 아이템", "공격력 +7", 7, 0, 2000) { Quantity = 0 }
        };
    }
        ...
        while (true)
        {
            ...
            if (int.TryParse(Console.ReadLine(), out int itemInput) && itemInput >= 0 && itemInput <= ownedItems.Count)
            {
                ...
                if (selectedItem.Quantity > 0)
                {
                    ...
                    if (selectedItem.Quantity == 0)
                    {
                        selectedItem.Purchase = false; // 수량이 0이면 다시 구매 가능하게 설정
                    }
                    inventory?.RemoveItem(selectedItem); // 인벤토리에서 아이템 제거
                }
                else
                {
                    Console.WriteLine("판매할 수량이 없습니다.");
                }
